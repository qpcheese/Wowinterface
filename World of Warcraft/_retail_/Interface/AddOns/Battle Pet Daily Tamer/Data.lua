...
tamer.dailyInfo = {
	...
	-- Khaz Algar
	[82292] = {223446,nil,2274,2552,650.8553,-1613.0017,2248,0.383,0.788,4,25,4561,4550,4549}, -- Collector Dyna (Isle of Dorn)
	[82291] = {223407,nil,2274,2552,3321.0750,-2453.5994,2248,0.482,0.320,4,25,4561}, -- Awakened Custodian (Isle of Dorn)
	[82293] = {223444,nil,2274,2601,2022.5596,-3792.8923,2214,0.616,0.376,4,25,4488,4553,4554}, -- Friendhaver Grem (The Ringing Deeps)
	[82300] = {222535,nil,2274,2601,1036.7715,-4173.0024,2214,0.669,0.581,4,25,4488}, -- Haywire Servobot (The Ringing Deeps)
	[82294] = {223442,nil,2274,2601,1465.7555,45.3999,2215,0.524,0.628,4,25}, -- Miniature Army (Hallowfall)
	[82298] = {223409,nil,2274,2601,1124.3491,-669.2299,2215,0.611,0.690,4,25}, -- The Thing from the Swamp (Hallowfall)
	[82295] = {223443,nil,2274,2601,-145.0814,-1743.9099,2256,0.613,0.369,4,25}, -- Ziriak (Azj-Kahet - Lower)
...
-- if on a Shadowlands client, remove Dragonflight stuff
if select(4,GetBuildInfo())<100000 then
	-- remove the Dragonflight targets
	for questID,info in pairs(tamer.dailyInfo) do
		if info[3]==1978 then
			tamer.dailyInfo[questID] = nil
		end
	end
	-- remove the Dragon Isles
	tamer.parentMapIDs[1978] = nil
end
...
tamer.azerothTransforms = {
	...
	[12] = {0.23932025419007, -77615.6918704, 0.5405578674487, -51854.886246381}, -- Kalimdor
...
-- azeroth map transformations on 11.0.2 TWW clients when Khaz Algar is shown
if select(4,GetBuildInfo())>=110002 then
	-- Kalimdor is nudged to the left a bit when Khaz Algar is shown on Azeroth map
	tamer.azerothTransforms[12] = {0.16032180497529, -77620.320592118, 0.54055777295856, -51854.908245911}
	-- Khaz Algar needs some special handling since the different maps don't seem to really belong to 2274; needs more work
	tamer.azerothTransforms[2274] = {0.22707557749186, -37540.978618015, 0.9248552156957, -26978.287752285}
end
