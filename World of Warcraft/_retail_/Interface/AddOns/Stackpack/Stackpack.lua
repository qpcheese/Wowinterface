...
local USE_BAG_UPDATE_DELAYED = true
...
-- Use official (as of Dragonflight) names for things where known.
...
local NUM_REAGENTBAG_SLOTS = NUM_REAGENTBAG_SLOTS or 0
...
local function GetItemLvl(itemLink)
	...
	ilvl =  ilvl
		or (GetDetailedItemLevelInfo and GetDetailedItemLevelInfo(itemLink))
...
local function GetItemOrPetInfo(link)
	...
	if ID then return GetItemInfo(link) end
	...
	if speciesID then
		...
		local petInfo = { petName, link, quality, petLevel, select(5, GetItemInfo(CAGED_ITEM_ID)) }
...
local function GetBaseItemLink(item)
	...
	local baseItemLink = id and select(2,GetItemInfo(id)) or item
	return baseItemLink
end
...
local _c_id = {
	...
	["Equipped"]	= -5,			-- -5  any number below -4 gives player equipment as a container
...
Debug("% tooltip: GetItem said id=",itemID,", data said id=",data["id"]," - activating countermeasures")
		...
		local actualLink = select(2, GetItemInfo(actualID))
		if actualLink then
			itemID = actualID
			itemLink = actualLink
Debug("% tooltip: replaced itemid/link with ",actualID,"/",actualLink)
	...
	local stacksize = StackpackSettings["ShowStacksizes"] and select(8, GetItemInfo(itemLink))
	if id or ilvl then
	...
	if GuildBankOpen then
		if StackpackSettings["StackGuildBank"] or GuildBankStackAll then
			...
			if (Count > 0) then
				...
				local Item, _, _, _, _, _, _, Stack = GetItemInfo(ID)
				itemCount = Count
				itemStack = Stack
				itemID = ID
				itemName = Item
...
local function doSPcycle(event)
	...
	if not SplitStack then
		...
		if (event == "BAG_UPDATE_DELAYED") or (SPCycle >= 3) then
			SPCycle = 0
			SPToggle = false
		end
	end
...
local function TriggerStacking(trigger)
	...
	elseif not OpenOrClose then
		Stacking = Stacking + ((GuildBankOpen and ((trigger=="GUILDBANKBAGSLOTS_CHANGED") and 1 or 0)) or ((trigger=="BAG_UPDATE_DELAYED") and BAG_UPDATE_DELAYED_VAL or 1))
	end
end
...
function Stackpack.OnLoad(self)
	...
	local _BAG_UPDATE_ = USE_BAG_UPDATE_DELAYED and "BAG_UPDATE_DELAYED" or "BAG_UPDATE"
	self:RegisterEvent(_BAG_UPDATE_)
