...
local function ExportData()
    ...
    for _, v in pairs(TradeLogDB) do
        ...
        for i = 1, 6 do
            ...
            if (playerItem) then
                table.insert(playerItems, "[" .. playerItem.name .. "(" .. playerItem.itemLink:match("item:(%d+):") ..
                                 ")]x" .. playerItem.numItems)
            end

            if (targetItem) then
                table.insert(targetItems, "[" .. targetItem.name .. "(" .. targetItem.itemLink:match("item:(%d+):") ..
                                 ")]x" .. targetItem.numItems)
            end
        end
