...
function t.options:Setup()
    ...
    if IsAddOnLoaded("BattlePetBinds") then
        self.info[4][3] = false
    end
    -- if Derangement Pet Battle Cooldowns or PetTracker is enabled, then default of EnemyAbilities should be false
    if IsAddOnLoaded("DerangementPetBattleCooldowns") or IsAddOnLoaded("PetTracker") then
        self.info[5][3] = false
    end
    ...
    self.version:SetText("version "..GetAddOnMetadata("BattlePetBattleUITweaks","Version"))
