...
do
	...
	XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffectController=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffectController
	...
	function class:LoadAll(scenario)
		...
		if(scenario.multiplayer)then
			...
			--if(true)then --!!!
				local defaults=XPRACTICE.AMIRDRASSILRAIDMAPS_CustomEffectDefaults.GetDefaults(index)
				for i=1,#defaults do
		...
		if(varname and index)then
			for i=1,#presets do
				...
				if(presetcode[1]~="END")then					
					...
					menubutton:SetText(XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.CodeToString(presetcode))
					local alpha=0
					...
					menubutton.displayobject.drawable:SetScript("OnClick",function(self,button,down)			
							if(button=="LeftButton" and down)then
								...
								if(not wmc.maineditbutton.selected)then
									for j=1,#wmc.presetbuttons do
										...
										if(i==j)then
											...
											if(wmc.presetbuttons[j].selected)then											
												scenario.statuslabel:SetText("You are now painting \""..XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.CodeToString(presetcode).."\".\nTo stop painting, click the same button again or right-click on the canvas.")
									...
									XPRACTICE.CustomEffectEditInterface_Statics.originaltext="\""..XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.CodeToString(presetcode).."\""
	...
	function class:SetGroundEffect(playerguid, c1,c2,c3,c4, x, y)
		...
		local effect=XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.MakeActorFromCode(self,code,x,y)
		effect.code=code
		self.scenario.statuslabel:SetText(self.scenario.multiplayer.guidpointers[playerguid].playername.." places \""..XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.CodeToString(code).."\".",3.0)
	...
	function class:SetAttachEffect(playerguid, c1,c2,c3,c4, attachguid)
		...
		if(string.sub(code,1,1)=="P")then
			if(string.sub(attachguid,1,4)=="ALL-")then			
				for k,v in pairs(self.scenario.multiplayer.allplayers) do						
					...
					local effect=XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.MakeActorFromCode(self,code,v.position.x,v.position.y)
					effect.attach=v				
				end
				self.scenario.statuslabel:SetText(self.scenario.multiplayer.guidpointers[playerguid].playername.." places \""..XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.CodeToString(code).."\".",3.0)
			else
		...
		elseif(string.sub(code,1,1)=="E")then
			if(string.sub(attachguid,1,4)=="ALL-")then			
				...
				if(bossindex)then
					...
					if(boss)then					
						local effect=XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.MakeActorFromCode(self,code,boss.position.x,boss.position.y)
						effect.attach=boss
						self.scenario.statuslabel:SetText(self.scenario.multiplayer.guidpointers[playerguid].playername.." places \""..XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffect.CodeToString(code).."\".",3.0)
					end
				end		
		...
		local plaintextcode=c1..c2..c3..tostring(string.byte(c4)-48)
		if(WeakAuras)then WeakAuras.ScanEvents("XP_AMIRDRASSIL_ATTACH_EFFECT",attachguid,plaintextcode) end	
	end
