do
	...
	do
		...
		XPRACTICE.SARKARETH2.Floor=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SARKARETH2.Floor
		...
		function class:Setup(environment,x,y,z)
			...
			self.instantbosses=true
		end
		
		function class:TransitionTo(index)
			if(XPRACTICE.SARKARETH2.FloorData.DrawMap[index])then
				...
				XPRACTICE.SARKARETH2.Bosses.CreateBosses(self.scenario,index)
			...
			if(self.transitionout)then
				...
				if(not stillinprogress)then	
					...
					if(self.transitionpending)then
						...
						if(XPRACTICE.SARKARETH2.FloorData.DrawMap[self.transitioninindex])then
							for i=1,#self.actors do
						...
						XPRACTICE.SARKARETH2.FloorData.DrawMap[self.transitioninindex](self.environment,self)
			...
			elseif(self.transitionpending)then
				if(self.transitioninindex==self.transitionpending)then										
					...
					self.transitioninindex=nil	
						...
						self.downstairs={}
						for i=1,#self.actors do
							tinsert(self.upstairs,self.actors[i])
						end
						local scale=1
						local actor=self:AddActorByFileID(1591934,0,0,-0.5,1.875)
						actor.scale=1.9
						actor:SetAlpha(0)
						tinsert(self.downstairs,actor)						
