do
	...
	do	
		...
		XPRACTICE.SARKARETH2.Spell_Pushback=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SARKARETH2.Spell_Pushback
		function class:SetCustomInfo()
		...
		function class:CastingAnimationFunction(spellinstancepointer)				
			if(spellinstancepointer:GetCastProgress()<=(1-2.5/6))then
				...
				spellinstancepointer.castercombat.mob.animationmodule:PlayAnimation(XPRACTICE.SARKARETH2.AnimationList.HurricaneWing)
			end
		end
		function class:StartCastingEffect(spellinstancepointer)
			...
			mob.combatmodule:ApplyAuraByClass(XPRACTICE.SARKARETH2.Aura_FreezeBoss_Pushback,mob.combatmodule,mob.environment.localtime)
		end
			
		function class:CompleteCastingEffect(castendtime,spellinstancepointer)	
			local mob=spellinstancepointer.castercombat.mob
			local push=XPRACTICE.SARKARETH2.PushbackHandler.new()
			push.scenario=spellinstancepointer.scenario
			push:Setup(mob.environment,mob.position.x,mob.position.y)
			
			local visual=XPRACTICE.SARKARETH2.ExtraPushbackFX.new()
	...
	do
		...
		XPRACTICE.SARKARETH2.Aura_FreezeBoss_Pushback = XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SARKARETH2.Aura_FreezeBoss_Pushback
	...
	do
		...
		XPRACTICE.SARKARETH2.ExtraPushbackFX = XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SARKARETH2.ExtraPushbackFX
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.SARKARETH2.PushbackHandler = XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SARKARETH2.PushbackHandler
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=2
			actor:SetModelByFileID(4067420)	--9fx_raid2_korrath_fatescribe_windradialground_state
		end
			...
			--for k,v in pairs(scenario.multiplayer.allplayers) do
				...
				if(not player:IsDeadInGame())then
					...
					-- if(not(#auras>0 and not auras[1].dying))then
						local auras2=player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.SARKARETH2.Aura_PushbackImmunity)
