do
	do
		...
		XPRACTICE.SARKARETH2.BreathManager=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SARKARETH2.BreathManager
		...
		function class:AddCircle(x,y)
			local scale=0.3
			local actor=self:AddActorByFileID(3088343,x,y,-0.35,scale)
			actor:SetSpellVisualKit(6527)
			actor.TEMP_x=x
			actor.TEMP_y=y
		end
				...
				if(scenario.player and scenario.player.phase==1 and not scenario.player:IsDeadInGame())then
					if(scenario.player.position.z>=0 and scenario.player.position.z<3)then
						...
						if(distsqr<=4*4)then
							...
							scenario:AttemptKillPlayer(scenario.player,false,false,"Stay out of Abyssal Breath.",3.0)
						end
					end
				end
