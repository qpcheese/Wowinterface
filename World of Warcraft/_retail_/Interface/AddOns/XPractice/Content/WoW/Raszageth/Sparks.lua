do
	...
	do	
		...
		XPRACTICE.RASZAGETH.Spell_Sparks=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASZAGETH.Spell_Sparks
		function class:SetCustomInfo()
		...
		function class:StartCastingEffect(spellinstancepointer)
			...
			for k,v in pairs(scenario.multiplayer.allplayers)do
				if(not v:IsDeadInGame())then
					local aura=v.combatmodule:ApplyAuraByClass(XPRACTICE.RASZAGETH.Aura_Spark,v.combatmodule,v.environment.localtime)
	...
	do
		...
		XPRACTICE.RASZAGETH.Aura_Spark=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASZAGETH.Aura_Spark
		function class:SetCustomInfo()
			super.SetCustomInfo(self)
		...
		function class:OnApply(localtime)
			...
			local gfx=XPRACTICE.RASZAGETH.SparkTelegraph.new()
	...
	do
		...
		XPRACTICE.RASZAGETH.SparkTelegraph=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASZAGETH.SparkTelegraph
	...
	function class.HandleCustom_SPARK_SPAWN(self,sender,args)		
		...
		if(player)then
			...
			local fx=XPRACTICE.RASZAGETH.SparkSpawnFX.new()
			fx:Setup(player.environment,args[1],args[2],1)
			local spark=XPRACTICE.RASZAGETH.VolatileSpark.new()
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASZAGETH.SparkSpawnFX=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASZAGETH.SparkSpawnFX
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=1
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASZAGETH.VolatileSpark=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASZAGETH.VolatileSpark
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=1
