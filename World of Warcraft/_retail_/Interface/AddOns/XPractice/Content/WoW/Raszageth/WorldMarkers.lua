...
do
	XPRACTICE.RASZAGETH.wmnumbers={{1,"BLUESQUARE"},{2,"GREENTRIANGLE"},{3,"PURPLEDIAMOND"},{4,"REDX"},{5,"YELLOWSTAR"},{6,"ORANGECIRCLE"},{7,"SILVERMOON"},{8,"WHITESKULL"}}
	XPRACTICE.RASZAGETH.wmmodels={REDX=456041,PURPLEDIAMOND=456039,GREENTRIANGLE=456037,YELLOWSTAR=456043,SILVERMOON=1014628,BLUESQUARE=456035,ORANGECIRCLE=1014619,WHITESKULL=1014641}

	XPRACTICE.RASZAGETH.wmtext={
	...
	XPRACTICE.RASZAGETH.WorldMarkerController=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.RASZAGETH.WorldMarkerController
	...
	function class:LoadAll(scenario,send)
		...
		if(scenario.multiplayer)then
			for index=1,8 do		--TODO: some code overlap with Handle_WORLDMARKER
				local wmname=XPRACTICE.RASZAGETH.wmnumbers[index][2]
				...
				if(x and y)then
					...
					obj.displayobject.drawable:SetModelByFileID(XPRACTICE.RASZAGETH.wmmodels[wmname])	-- raid_ui_fx_
	...
	function class:QuickButtons(x,y,scenario)
		...
		for i=1,8 do
			...
			menubutton.icontext=XPRACTICE.RASZAGETH.wmtext[XPRACTICE.RASZAGETH.wmnumbers[i][2]]
			menubutton:SetText("")	
			local alpha=0
