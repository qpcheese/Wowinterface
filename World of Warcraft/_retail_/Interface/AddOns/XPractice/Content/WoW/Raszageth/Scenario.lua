...
-- maybe use one of the "beam" effects
-- lightingbeam_neutral_state_base.m2 ? 0,158,159 (requires pitch tilt)
-- 8fx_kingmecha_cuttingbeam_large.m2 ? 0,158,159 (158 might not show beam though)

-- lightning_halo_missile.m2 -- 1/8th circle, requires manual scaling
-- ul_lightning_tunnel.m2 ?

-- consider using shaman_lightningcall (flashing lights warning though)
-- dragonbreath_leishen_lightning_withdecay 0 158 159 probably not, too much smoke
-- leishen_lightning_precast 0,158,159 maybe
-- leishen_lightning_whip_state 0,158 maybe (short, awkward offset)

---- probably best to use wowexport to browse unlabeled files...
-- spell effects might be hiding in Models / Unk_exp09_4631242 and thereabouts
-- 4550332 	convincing lightning breath substitute (0,158,159)
-- 4550333 lightning chargeup air-inwards

-- 4612922 very possibly margok stormnova AOE splash
-- 4618192 lightning ring rim -- 0 158 159 -- unusually small, probably not rasza

-- 4631248: static field spark circle with larger center spark.  no swirl/fillcolor
-- 4901597 superficially resembles final phase soak sparks?
-- 4689429, 4689441 rotating vault machinery?
-- 4696086 rotating vault machinery under translucent floor panel (good luck fitting in with current map though)

-- 4623330 very large floating containment rings

-- Combat stance: ReadyUnarmed [5]
-- Autoattacks: AttackUnarmed [0] and [1]
-- Tankbuster: AttackUnarmed [2]
--			(cast bar, but no apparent special cast animation)
--
-- Breath attack: ReadySpellDirected [100] -> SpellCastDirected [99] -> ChannelCastDirected [98]
-- 												99 only plays for a split second
--
-- Knockback attack: CustomSpell02 [101]
--		(the animation is the same length every time, even if the knockback lasts twice as long)unjk
--
-- no boss animations for bombs or sparks
--

do
	local super=XPRACTICE.Scenario
	XPRACTICE.RASZAGETH.Scenario=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.RASZAGETH.Scenario

	XPRACTICE.RASZAGETH.NPCID=199031
	

	function class:SetCustomInfo()
		super.SetCustomInfo(self)
		
		self.savedataname="RASZAGETH"
		self.npcid=XPRACTICE.RASZAGETH.NPCID
	...
	function class:Populate()
		...
		self.collision=XPRACTICE.RASZAGETH.Collision.new()
		...
		player.position={x=46.6,y=27.3,z=0}
		...
		-- local debufficon=XPRACTICE.RASZAGETH.ConductiveMarkDebuffIcon.new()
		...
		-- local debufficon=XPRACTICE.RASZAGETH.PrimalBlizzardDebuffIcon.new()
		-- debufficon:Setup(self.game.environment_gameplay)
		...
		-- local boss=XPRACTICE.RASZAGETH.TheJailer.new()
		-- self.boss=boss	
		...
		-- self.layout=XPRACTICE.RASZAGETH.Layout.new()
		-- self.layout:Setup(self.game.environment_gameplay,self)
		
		local floorobj
		floorobj=XPRACTICE.RASZAGETH.Floor.new()
		...
		self.worldmarkercontroller=XPRACTICE.RASZAGETH.WorldMarkerController.new()
		self.worldmarkercontroller:Setup(self.game.environment_gameplay)
			
		
		self.floor:TransitionTo(XPRACTICE.RASZAGETH.FloorData.RASZAGETH)		
		local scenario=self
		...
		self.multiplayer=XPRACTICE.RASZAGETH.Multiplayer.new()	-- inherits multiplayerv2
		...
		button=class.QuickCollapseButton(self.game.environment_gameplay,self,buttongroup,"Boss controls",
					"\n\n\n\n\nUse the buttons to practice individual mechanics,\n"..
					"or click Play to run through the entirety of Phase 1.\n"..
					"(If you are playing solo or without tanks in your group,\n"..
					"first use the Tank Buttons menu to move Raszageth to the edge of the arena.)\n",
					30.0)
		...
		-- XPRACTICE.OptionsMenuButton.debugbuttony=XPRACTICE.OptionsMenuButton.debugbuttony+45		
		-- local changemapbuttons=XPRACTICE.RASZAGETH.ChangeMap.QuickButtons(156,310,self)
		...
		--self.tauntbutton=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_TauntBoss,self.spellbunny,"TAUNT\nBOSS")
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
		self.brezbutton=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_ChangeMap,self.spellbunny,"TEST")
		
		--self.roarbutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_Roar,self.spellbunny,"ROAR")
		--self.pallybubblebutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_PallyBubble,self.spellbunny,"INVUL\n5 SEC")

		local button
		XPRACTICE.BossDebugButton.CarriageReturn()
		XPRACTICE.BossDebugButton.debugbuttony=286+45	
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52+52
		...
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52		
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52		
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_SetTanks,self.spellbunny,"SET\nTANKS",nil)
		tinsert(bosscontrolbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_Tankbuster,self.bosses[1],"TANK\nBUSTER",9,nil,"tankbuster")
		tinsert(bosscontrolbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_Bombs,self.bosses[1],"BOMBS",9,nil,"bombs")
		tinsert(bosscontrolbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_Breath,self.bosses[1],"BREATH",9,nil,"breath")
		tinsert(bosscontrolbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_Sparks,self.bosses[1],"SPARKS",9,nil,"sparks")
		tinsert(bosscontrolbuttons,button)		
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_Pushback,self.bosses[1],"PUSH\nBACK",nil,nil,"pushback")
		tinsert(bosscontrolbuttons,button)			
		...
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52		
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_MoveBoss,self.spellbunny,"MOVE\nBOSS")
		tinsert(tankbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_TauntBoss,self.spellbunny,"TAUNT\nBOSS")
		tinsert(tankbuttons,button)
		-- button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_EarthenPillar,self.bosses[1],"EARTH\nPILLAR",9,nil,"earthenpillar")
		-- tinsert(tankbuttons,button)
		-- button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_ConductiveMark,self.bosses[1],"MARK",nil,nil,"conductivemark")
		-- tinsert(tankbuttons,button)
		-- button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_MeteorAxe,self.bosses[1],"METEO\nAXE",nil,nil,"meteoraxe")
		-- tinsert(tankbuttons,button)		
		-- button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_PrimalBlizzard,self.bosses[1],"BLIZZ",nil,nil,"primalblizzard")
		-- tinsert(tankbuttons,button)				
		-- button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_ClearAll,self.spellbunny,"CLEAR")
		tinsert(tankbuttons,button)
		...
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_CallToArms,self.spellbunny,"CALL")
		tinsert(extrabuttons,button)
		...
		self.brezbutton=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_Brez,self.spellbunny,"ANKH")		
		
		

		--self.phase1button=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_ResetPhase1,self.spellbunny,"GOTO\nPHASE1",9)
		
		
		-- self.swirlsyncstopbutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_SwirlSync_Stop,self.spellbunny,"SWIRL\nSTOP")
		-- self.resetstacksbutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_ResetStacks,self.spellbunny,"RESET\nSTACK")
		
		-- XPRACTICE.BossDebugButton.CarriageReturn()
		-- self.humanappearancebutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_HumanAppearance,self.spellbunny,"HMN")
		-- self.lichappearancebutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.RASZAGETH.Spell_LichAppearance,self.spellbunny,"LICH")		
		-- XPRACTICE.BossDebugButton.CarriageReturn()
		...
		self.schedule=XPRACTICE.RASZAGETH.Schedule.new()
		...
		-- ActionButton_ShowOverlayGlow(self.setrolebutton.displayobject.drawable)
			...
			-- local temp=XPRACTICE.RASZAGETH.SparkTelegraph.new()
		...
		self.timestamps=XPRACTICE.RASZAGETH.SetupTimestamps(self)
		XPRACTICE.RASZAGETH.SetupPlayPauseButtons(self)
	...
	function class:Step(elapsed)
		...
		if(self.player)then
			...
			----DebuffCheck is moved to Aura_ConductiveMark for now
	...
	function class:Brez()
		local auras=self.player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.RASZAGETH.Aura_ChangePhaseForced)
		if(#auras>0)then 			
			return 
		end				
	...
	function class:BrezOther(player)
		local auras=player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.RASZAGETH.Aura_ChangePhaseForced)
		if(#auras>0)then 			
			return 
		end				
	...
	function class:AttemptKillPlayer(player,ignorebubble)
		...
		local auras=player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.RASZAGETH.Aura_PallyBubble)
	...
	function class:PlaceHintArrow(x,y)
		...
		local arrow=XPRACTICE.RASZAGETH.HintArrow.new()
	...
	function class:PlaceBossHintArrow(x,y)
		...
		local arrow=XPRACTICE.RASZAGETH.BossHintArrow.new()
	...
	XPRACTICE.ScenarioList.RegisterScenario(XPRACTICE.RASZAGETH.Scenario,"The Winds of Change","(Mythic Raszageth Phase 1)","SCENARIO")
	XPRACTICE.ScenarioList.RegisterScenario(XPRACTICE.RASZAGETH.Scenario,"10.0","Vault of the Incarnates","SEPARATOR")
end
