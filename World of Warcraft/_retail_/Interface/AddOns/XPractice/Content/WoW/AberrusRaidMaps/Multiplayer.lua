do	
	...
	local roomnumber=XPRACTICE.ABERRUSRAIDMAPS.NPCID
	local roomBCD=XPRACTICE.MULTIPLAYERV2.Datatypes.BCD.ToString(XPRACTICE.ABERRUSRAIDMAPS.NPCID)	
	...
	XPRACTICE.ABERRUSRAIDMAPS.Multiplayer=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.ABERRUSRAIDMAPS.Multiplayer		
	...
	function class.Handle_HELLO(self,sender,args)
		...
		--print(XPRACTICE.MULTIPLAYERV2.Multiplayerv2.Handle, XPRACTICE.ABERRUSRAIDMAPS.Handle)
	...
	function class.HandleCustom_SPRINT(self,sender,args)
		...
		local visual=XPRACTICE.ABERRUSRAIDMAPS.SprintVisualEffect.new()
	...
	function class.HandleCustom_ROAR(self,sender,args)
		...
		if(distsqr<=radius*radius)then
			...
			local visual=XPRACTICE.ABERRUSRAIDMAPS.RoarVisualEffect.new()
	...
	function class.HandleCustom_PALLYBUBBLE(self,sender,args)
		...
		if(#auras==0)then
			local aura=player.combatmodule:ApplyAuraByClass(XPRACTICE.ABERRUSRAIDMAPS.Aura_PallyBubble)
		end
		local auras=player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.ABERRUSRAIDMAPS.Aura_Freeze)
		for i=1,#auras do
			auras[i]:Die()
		end
	end	
	...
	function class.HandleCustom_CHANGEMAP(self,sender,args)
		...
		local name=tostring(XPRACTICE.ABERRUSRAIDMAPS.ChangeMap.BossNames[i])
		if(name)then
