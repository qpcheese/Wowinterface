do
	...
	do	
		...
		XPRACTICE.RASHOK.Spell_Pushback=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.Spell_Pushback
		function class:SetCustomInfo()
		...
		function class:CastingAnimationFunction(spellinstancepointer)				
			if(spellinstancepointer:GetCastProgress()<=(1-2.5/6))then
				...
				spellinstancepointer.castercombat.mob.animationmodule:PlayAnimation(XPRACTICE.RASHOK.AnimationList.HurricaneWing)
			end
		end
	...
	do
		...
		XPRACTICE.RASHOK.Aura_FreezeBoss_Pushback = XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.Aura_FreezeBoss_Pushback
	...
	do
		...
		XPRACTICE.RASHOK.ExtraPushbackFX = XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.ExtraPushbackFX
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.PushbackHandler = XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.PushbackHandler
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=2
			actor:SetModelByFileID(4067420)	--9fx_raid2_korrath_fatescribe_windradialground_state
		end
			...
			--for k,v in pairs(scenario.multiplayer.allplayers) do
				...
				if(not player:IsDeadInGame())then
					...
					-- if(not(#auras>0 and not auras[1].dying))then
						local auras2=player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.RASHOK.Aura_PushbackImmunity)
