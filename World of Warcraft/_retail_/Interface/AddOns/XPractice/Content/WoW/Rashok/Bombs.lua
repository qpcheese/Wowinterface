do
	...
	do	
		...
		XPRACTICE.RASHOK.Spell_Bombs=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.Spell_Bombs
		function class:SetCustomInfo()
		...
		function class:CompleteCastingEffect(castendtime,spellinstancepointer)	
			...
			for i=1,3 do
				...
				if(target and not target:IsDeadInGame())then
					local aura=target.combatmodule:ApplyAuraByClass(XPRACTICE.RASHOK.Aura_Bomb,target.combatmodule,target.environment.localtime)
	...
	do
		...
		XPRACTICE.RASHOK.Aura_Bomb=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.Aura_Bomb
		function class:SetCustomInfo()
			super.SetCustomInfo(self)
		...
		function class:OnApply(localtime)
			...
			local gfx=XPRACTICE.RASHOK.BombStatic.new()
			gfx:Setup(player.environment,player.position.x,player.position.y,0.1)
			gfx.aura=self
			gfx.player=player
			local gfx=XPRACTICE.RASHOK.BombRing.new()
			gfx:Setup(player.environment,player.position.x,player.position.y,0.1)
			gfx.aura=self
			gfx.player=player		
			local gfx=XPRACTICE.RASHOK.BombFalloffWarning.new()
			gfx:Setup(player.environment,player.position.x,player.position.y,0.1)
			gfx.aura=self
			gfx.player=player				
		end
	...
	do
		...
		XPRACTICE.RASHOK.BombFalloffWarning=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BombFalloffWarning
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.BombRing=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BombRing
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=4.0
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.BombStatic=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BombStatic
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=2.0
			--actor:SetModelByFileID(4254394)	--target_lightning_state_rimonly
		...
		function class:Step(elapsed)
			...
			if(self.aura.dead)then
				if(not self.despawning)then
					...
					local gfx=XPRACTICE.RASHOK.BombExplosion1.new()
					gfx:Setup(self.environment,self.position.x,self.position.y,0.1)
					local gfx=XPRACTICE.RASHOK.BombExplosion2.new()
					gfx:Setup(self.environment,self.position.x,self.position.y,0.1)
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.BombExplosion1=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BombExplosion1
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=4.0
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.BombExplosion2=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BombExplosion2
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=4.0
