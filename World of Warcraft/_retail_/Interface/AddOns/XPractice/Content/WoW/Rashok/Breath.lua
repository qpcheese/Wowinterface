do
	...
	do	
		...
		XPRACTICE.RASHOK.Spell_Breath=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.Spell_Breath
		function class:SetCustomInfo()
		...
		function class:CastingAnimationFunction(spellinstancepointer)	
			...
			if(spellinstancepointer:GetCastProgress()<0.8)then
				...
				spellinstancepointer.castercombat.mob.animationmodule:PlayAnimation(XPRACTICE.RASHOK.AnimationList.SpellCastDirected)
			end
		end
		function class:StartCastingEffect(spellinstancepointer)
			...
			local gfx=XPRACTICE.RASHOK.ChargeUpEffect.new()
		...
		function class:CompleteCastingEffect(castendtime,spellinstancepointer)	
			...
			local offset=260
			local gfx=XPRACTICE.RASHOK.BreathFX.new()
			...
			local offset=50
			local gfx=XPRACTICE.RASHOK.BreathFX2.new()			
			gfx:Setup(mob.environment,mob.position.x+offset*math.cos(t),mob.position.y+offset*math.sin(t),5.5)
			gfx.orientation_displayed.yaw=t
			-- local offset=25
			-- local gfx=XPRACTICE.RASHOK.BreathFX3.new()
			-- gfx:Setup(mob.environment,mob.position.x+offset*math.cos(t),mob.position.y+offset*math.sin(t),5.5)
			-- gfx.orientation_displayed.yaw=t
	...
	do
		...
		XPRACTICE.RASHOK.ChargeUpEffect=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.ChargeUpEffect
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.BreathFX=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BreathFX
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=2.0
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.BreathFX2=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BreathFX2
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=4.0
	...
	do
		local super=XPRACTICE.WoWObject
		XPRACTICE.RASHOK.BreathFX3=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.RASHOK.BreathFX3
		function class:SetActorAppearanceViaOwner(actor)
			self.scale=2.0
