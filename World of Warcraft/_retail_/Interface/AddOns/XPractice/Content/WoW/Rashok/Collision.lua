do
	...
	XPRACTICE.RASHOK.Collision = {}
	local class=XPRACTICE.RASHOK.Collision
	class.__index = class
	...
	function class:OutOfBoundsCheck()
		...
		if(scenario.player.position.z<-60)then	--TODO: determine most-appropriate height for death plane
			...
			local auras=player.combatmodule.auras:GetAurasByClassIfExistIncludingNewAuras(XPRACTICE.RASHOK.Aura_ExplosiveRunes)	
			for i=1,#auras do
				auras[i]:OnRemove()
			end
