do
	...
	do	
		...
		XPRACTICE.SMOLDERON.Spell_SetRole=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_SetRole
	...
	do	
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_SetSoak=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_SetSoak
	...
	do	
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_Brez=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_Brez
	...
	do
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_PallyBubble=XPRACTICE.inheritsFrom(super)
		
		local class=XPRACTICE.SMOLDERON.Spell_PallyBubble
			function class:SetCustomInfo()
			super.SetCustomInfo(self)
			self.name="(Bubble)"
	...
	do
		...
		XPRACTICE.SMOLDERON.Aura_PallyBubble = XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Aura_PallyBubble
		...
		function class:OnApply(localtime)
			...
			local visual=XPRACTICE.SMOLDERON.PallyBubbleVisual.new()
	...
	do
		...
		XPRACTICE.SMOLDERON.PallyBubbleVisual=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.PallyBubbleVisual
	...
	do	
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_ChangeMap=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_ChangeMap
	...
	do	
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_CallToArms=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_CallToArms
		...
		function class:CompleteCastingEffect(castendtime,spellinstancepointer)	
			...
			if((not scenario.tempmenu.currentmenu) or scenario.tempmenu.currentmenu.menuclass~=XPRACTICE.CallToArmsInterface)then
				...
				scenario.tempmenu.currentmenu.extradata="CALL_TO_ARMS JOIN SMOLDERON 1"
			else
				scenario.tempmenu:UnloadCurrentMenu()
			end			
	...
	do	
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_MoveBoss=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_MoveBoss
		...
		function class:Broadcast_v2(scenario,bossindex,x,y)
			if(not XPRACTICE.MULTIPLAYERV2.IsRealOfficer("player"))then
				scenario.statuslabel:SetText("That action requires raid lead/assist.",3.0)
				return
			end		
			if(bossindex==nil)then
				--then we must have clicked on it instead. TODO: do better
				self:CompleteCastingEffect(0,self)
				return
			end
			local args={}
			args[1]=string.char(bossindex)
			args[2]=x
			args[3]=y
			self.scenario.multiplayer:FormatAndSendCustom("MOVE_BOSS",unpack(args))			
		end		
	...
	do	
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_SetTanks=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_SetTanks
			...
			if(boss)then				
				...
				picker.callback=function(s)
					...
					for k,v in pairs(scenario.multiplayer.allplayers) do
						local distx=picker.position.x-v.position.x
						local disty=picker.position.y-v.position.y
					...
					if(target)then
						if(target.markedastank)then
							target.markedastank=false
							scenario.statuslabel:SetText(target.playername.." is no longer marked as tank for boss targeting purposes.")
						else
							target.markedastank=true
							scenario.statuslabel:SetText(target.playername.." is now marked as tank for boss targeting purposes.\n(At the moment, this setting will not save when you close the scenario.)")
						end
					else
						scenario.statuslabel:SetText("Couldn't find a player near where you clicked.")
					end
				end
			end
		end
	...
	do	
		local super=XPRACTICE.Spell
		XPRACTICE.SMOLDERON.Spell_ClearAll=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.SMOLDERON.Spell_ClearAll
		function class:SetCustomInfo()
			super.SetCustomInfo(self)
			self.name="(Clear)"
	...
	local roomnumber=XPRACTICE.SMOLDERON.NPCID
	local roomBCD=XPRACTICE.MULTIPLAYERV2.Datatypes.BCD.ToString(XPRACTICE.SMOLDERON.NPCID)		
	do
	...
	local class=XPRACTICE.SMOLDERON.Multiplayer	
	function class.HandleCustom_CLEAR_ALL(self,sender,args)		
		local scenario=self.scenario
		for k,v in pairs(scenario.multiplayer.allplayers)do
			local auras=v.combatmodule.auras:GetAurasByClassIfExistIncludingNewAuras(XPRACTICE.SMOLDERON.Aura_ConductiveMark)
			for i=1,#auras do
				auras[i]:Die()
			end
		end		
