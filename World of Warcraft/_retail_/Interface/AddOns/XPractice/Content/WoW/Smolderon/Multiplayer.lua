do	
	...
	do
		...
		messagecode=messagecode+1
		local BCDmessagecode=XPRACTICE.MULTIPLAYERV2.Datatypes.ShortBCD.ToString(messagecode)
		local messagename="WORLD_IN_FLAMES"
		local super=XPRACTICE.MULTIPLAYERV2.Message
		local lambda=XPRACTICE.inheritsFrom(super)
		local class=lambda	
		function class:SetCustomInfo()
			...
			self.ignoreself=false
			self.requireofficer=true
			self.argtypes={{XPRACTICE.MULTIPLAYERV2.Datatypes.CHAR}, --pattern
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 1-1
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 1-2
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 2-1
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 2-2
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 3-1
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 3-2
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 4-1
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 4-2
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 5-1
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--ring radius 5-2
					{XPRACTICE.MULTIPLAYERV2.Datatypes.BCD,4},	--player hitbox radius
				}	
		...
		messagecode=messagecode+1
		local BCDmessagecode=XPRACTICE.MULTIPLAYERV2.Datatypes.ShortBCD.ToString(messagecode)
		local messagename="DEAD_WORLD_IN_FLAMES"
		local super=XPRACTICE.MULTIPLAYERV2.Message
		local lambda=XPRACTICE.inheritsFrom(super)
		local class=lambda	
		function class:SetCustomInfo()
		...
		messagecode=messagecode+1
		local BCDmessagecode=XPRACTICE.MULTIPLAYERV2.Datatypes.ShortBCD.ToString(messagecode)
		local messagename="EMBARRASSED_MOTES"
		local super=XPRACTICE.MULTIPLAYERV2.Message
		local lambda=XPRACTICE.inheritsFrom(super)
		local class=lambda	
		function class:SetCustomInfo()
	...
	XPRACTICE.SMOLDERON.Multiplayer=XPRACTICE.inheritsFrom(super)
	...
	function class.Handle_HELLO(self,sender,args)
		...
		--print(XPRACTICE.MULTIPLAYERV2.Multiplayerv2.Handle, XPRACTICE.SMOLDERON.Handle)
	...
	function class.HandleCustom_CHANGEMAP(self,sender,args)
		...
		local name=tostring(XPRACTICE.SMOLDERON.ChangeMap.BossNames[i])
		if(name)then
	...
	function class.HandleCustom_WORLD_IN_FLAMES(self,sender,args)
		local scenario=self.scenario
		local i=string.byte(args[1])
		
		if(self.roomlocked)then
			scenario:ResetTempVars()
			local pattern=string.byte(args[1])
			scenario.schedule:SetupEverything(pattern)
			XPRACTICE.SMOLDERON.RING_RADIUS[1]={args[2],args[3]}
			XPRACTICE.SMOLDERON.RING_RADIUS[2]={args[4],args[5]}
			XPRACTICE.SMOLDERON.RING_RADIUS[3]={args[6],args[7]}
			XPRACTICE.SMOLDERON.RING_RADIUS[4]={args[8],args[9]}
			XPRACTICE.SMOLDERON.RING_RADIUS[5]={args[10],args[11]}
			XPRACTICE.SMOLDERON.PLAYER_HITBOX_RADIUS=args[12]
			scenario.pauseallmechanics=false
			scenario:SwitchToStopButton()			
		end
	end		
	...
	function class.HandleCustom_DEAD_WORLD_IN_FLAMES(self,sender,args)
		local scenario=self.scenario
		local player=self.allplayers[sender]	
		if(not player)then return end
		--if(player.paddedguid)then scenario.guiddebuffs[player.paddedguid]=nil end
	...
	function class.HandleCustom_EMBARRASSED_MOTES(self,sender,args)
		local scenario=self.scenario
		local player=self.allplayers[sender]	
		if(not player)then return end
		--if(player.paddedguid)then scenario.guiddebuffs[player.paddedguid]=nil end
		if(player:IsDeadInGame())then return end		
		--scenario:AttemptKillPlayer(player,true,true,player.playername.." missed a Living Flame mote.",5.0)
		player:CreateSpeechBubble("I missed a mote!",5.0)
	end		
	
	function class.HandleCustom_ROLE(self,sender,args)
		local scenario=self.scenario
		local player=self.allplayers[sender]	
		if(not player)then return end
		--if(player.paddedguid)then scenario.guiddebuffs[player.paddedguid]=nil end
		if(player:IsDeadInGame())then return end		
		local role="Unknown"
		if(string.byte(args[1])==1)then role="Tank"
		elseif(string.byte(args[1])==2)then role="Melee"
		elseif(string.byte(args[1])==3)then role="Ranged"
		end
		player.TEMP_role=role
		scenario.statuslabel:SetText(player.playername.." is now "..role..".")
	end			
