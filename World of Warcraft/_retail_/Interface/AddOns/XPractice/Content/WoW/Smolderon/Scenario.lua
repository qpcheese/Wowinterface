...
do
	...
	XPRACTICE.SMOLDERON.Scenario=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.SMOLDERON.Scenario

	XPRACTICE.SMOLDERON.NPCID=214082
	

	function class:SetCustomInfo()
		super.SetCustomInfo(self)
		
		self.savedataname="SMOLDERON"
		self.npcid=XPRACTICE.SMOLDERON.NPCID
		...
		self.tempobjs={}
		self.motes={}
		self.groups_have_been_set=false
		self.group1tank=nil
		self.group2tank=nil
		self.group1={}
		self.group2={}
	...
	function class:Populate()
		...
		--self.game.window.bordergraphic.titletext.fontstring:SetText("X Practice (.........)")
		...
		self.collision=XPRACTICE.SMOLDERON.Collision.new()
		...
		player.orientation.yaw=180/180*math.pi;player.orientation_displayed.yaw=player.orientation.yaw		

		player.scenario=self
		self.player=player		
		tinsert(self.allplayers,player)
		
		self.spellfxmanager=XPRACTICE.SMOLDERON.SpellFXManager.new()
		self.spellfxmanager:Setup(self.game.environment_gameplay)
		self.spellfxmanager.scenario=self
		...
		-- local debufficon=XPRACTICE.SMOLDERON.ConductiveMarkDebuffIcon.new()
		...
		local floorobj
		floorobj=XPRACTICE.SMOLDERON.Floor.new()
		...
		self.worldmarkercontroller=XPRACTICE.SMOLDERON.WorldMarkerController.new()
		self.worldmarkercontroller:Setup(self.game.environment_gameplay)
			
		self.customeffectcontroller=XPRACTICE.AMIRDRASSILRAIDMAPS.CustomEffectController.new()
		self.customeffectcontroller.scenario=self
		self.customeffectcontroller:Setup(self.game.environment_gameplay)
					
		
		self.floor:TransitionTo(XPRACTICE.AMIRDRASSILRAIDMAPS.FloorData.SMOLDERON)		
		local scenario=self
		...
		self.multiplayer=XPRACTICE.SMOLDERON.Multiplayer.new()	-- inherits multiplayerv2
		...
		XPRACTICE.OptionsMenuButton.debugbuttony=XPRACTICE.OptionsMenuButton.debugbuttony+45
		local customeffectbuttons=self.customeffectcontroller:QuickButtons(156,310,self)
		button=class.QuickCollapseButton(self.game.environment_gameplay,self,buttongroup,"Effects",
					"Welcome to the new draw-on-the-map interface!\n\nThese buttons require officer privileges.\nPlease make sure all players are in the room before you start drawing.\nPlayers who join later will not see anything you have already drawn.",
					30.0)
		self.customeffectbutton=button
		self.customeffectbutton.buttons=customeffectbuttons
		
		XPRACTICE.OptionsMenuButton.debugbuttony=XPRACTICE.OptionsMenuButton.debugbuttony+45
		local autoeffectbuttons={}
		button=class.QuickCollapseButton(self.game.environment_gameplay,self,buttongroup,"Group Effects",
					"\n\n\n\nPhase 1 dance test for multiplayer. Please follow the directions carefully:\n\n"..
					"1. ALL players click TANK, MELEE, or RANGE while the raid officer is in the room.\n"..
					"2. Raid officer clicks RNG to confirm groups.\n"..
					"3. Raid officer clicks GROUP 1 or GROUP 2 to place circles.\n"..
					"(*Everyone must stand still so previous group's circles can clear properly.)\n"..
					"4. Dance!\n"..
					"5. If anyone changes role, raid officer must click RNG again to redo groups.",
					30.0)
		self.autoeffectbutton=button
		self.autoeffectbutton.buttons=autoeffectbuttons
		
		XPRACTICE.OptionsMenuButton.debugbuttony=XPRACTICE.OptionsMenuButton.debugbuttony+45
		local forcesingleplayerbuttons={}
		button=class.QuickCollapseButton(self.game.environment_gameplay,self,buttongroup,"Solo Mode",
					"This button will switch you to single-player mode from within a raid group,\n"..
					"allowing you to practice mechanics solo without needing officer permissions.\n"..
					"To revert to multiplayer mode, you'll need to restart the scenario.\n",
					30.0)
		self.forcesingleplayerbutton=button
		self.forcesingleplayerbutton.buttons=forcesingleplayerbuttons		
		
		
		-- XPRACTICE.OptionsMenuButton.debugbuttony=XPRACTICE.OptionsMenuButton.debugbuttony+45		
		-- local changemapbuttons=XPRACTICE.SMOLDERON.ChangeMap.QuickButtons(156,310,self)
		...
		-- XPRACTICE.BossDebugButton.debugbuttony=XPRACTICE.BossDebugButton.debugbuttony+52
		-- XPRACTICE.BossDebugButton.debugbuttony=XPRACTICE.BossDebugButton.debugbuttony+52
		-- button=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.LEGACY.Spell_GetPlayerPosition,self.spellbunny)
		...
		XPRACTICE.BossDebugButton.debugbuttony=332+45
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52		
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_ForceSingleplayer,self.spellbunny,"SOLO\nMODE")
		tinsert(forcesingleplayerbuttons,button)
		XPRACTICE.BossDebugButton.CarriageReturn()
		
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52		
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52		

		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Role_Tank,self.spellbunny,"TANK")
		tinsert(autoeffectbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Role_Melee,self.spellbunny,"MELEE")
		tinsert(autoeffectbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Role_Ranged,self.spellbunny,"RANGE",9)
		tinsert(autoeffectbuttons,button)

		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52		
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Reroll,self.spellbunny,"RNG")
		tinsert(autoeffectbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Group1Effects,self.spellbunny,"GROUP\n1",9)
		tinsert(autoeffectbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Group2Effects,self.spellbunny,"GROUP\n2",9)
		tinsert(autoeffectbuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_ClearEffects,self.spellbunny,"CLEAR")
		tinsert(autoeffectbuttons,button)

		XPRACTICE.BossDebugButton.CarriageReturn()				

		--self.tauntbutton=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_TauntBoss,self.spellbunny,"TAUNT\nBOSS")
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
		XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
			XPRACTICE.BossDebugButton.debugbuttonx=XPRACTICE.BossDebugButton.debugbuttonx+52
		self.brezbutton=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_ChangeMap,self.spellbunny,"TEST")		
		
		--self.roarbutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Roar,self.spellbunny,"ROAR")
		--self.pallybubblebutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_PallyBubble,self.spellbunny,"INVUL\n5 SEC")
		...
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_MoveBoss,self.spellbunny,"MOVE\nBOSS")
		...
		-- tinsert(extrabuttons,button)
		button=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_CallToArms,self.spellbunny,"CALL")
		tinsert(extrabuttons,button)
		...
		self.brezbutton=XPRACTICE.BossDebugButton_v2.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_Brez,self.spellbunny,"ANKH")		
		
		

		--self.phase1button=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_ResetPhase1,self.spellbunny,"GOTO\nPHASE1",9)
		
		
		-- self.swirlsyncstopbutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_SwirlSync_Stop,self.spellbunny,"SWIRL\nSTOP")
		-- self.resetstacksbutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_ResetStacks,self.spellbunny,"RESET\nSTACK")
		
		-- XPRACTICE.BossDebugButton.CarriageReturn()
		-- self.humanappearancebutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_HumanAppearance,self.spellbunny,"HMN")
		-- self.lichappearancebutton=XPRACTICE.BossDebugButton.QuickButton(self.game.environment_gameplay,self,XPRACTICE.SMOLDERON.Spell_LichAppearance,self.spellbunny,"LICH")		
		-- XPRACTICE.BossDebugButton.CarriageReturn()
		...
		self.roles={}		
			...
			-- local obj=XPRACTICE.WoWObject.new()
			-- obj:Setup(self.game.environment_gameplay,0,0,0.03)
			-- obj.displayobject.drawable:SetModelByFileID(5333970)
			-- --obj.displayobject.drawable:SetModelByFileID(5345129)
			-- obj.displayobject.drawable:SetShown(true)
			-- obj.displayobject.drawable:SetAlpha(1)
			-- --obj.displayobject.drawable:SetAnimation(159)
			-- obj.Draw=function()end

		--!!!
		-- for i=0,11 do
			-- local obj=XPRACTICE.WoWObject.new()
			-- obj:Setup(self.game.environment_gameplay,15*math.cos(i/12*math.pi*2),15*math.sin(i/12*math.pi*2),0.03)
			-- obj.displayobject.drawable:SetModelByFileID(5372693)
			-- obj.displayobject.drawable:SetShown(true)			
			-- obj.displayobject.drawable:SetAlpha(1)
			-- obj.displayobject.drawable:SetScale(2)
			-- --obj.displayobject.drawable:SetAnimation(127)
			-- obj.Draw=function()end
		-- end
		...
		self.schedule=XPRACTICE.SMOLDERON.Schedule.new()
	...
	function class:ResetTempVars()	
		...
		self.phasetime=0
		self.state=1
		self.spellfxmanager:Reset()
		self.bosses[1].animationmodule=XPRACTICE.AnimationModule.new()
		self.bosses[1].animationmodule:Setup(self.bosses[1])
	...
	function class:SetupNormalButtons()
		local scenario=self
		self.statuslabel:SetText("",10.0)
		self.startbuttons={}
		for i=1,3 do
			local menubutton
			menubutton=XPRACTICE.Button.new()
			menubutton:Setup(self.game.environment_gameplay)		
			menubutton.position={x=self.game.SCREEN_WIDTH/2-200+200*(i-1)-150/2,y=self.game.SCREEN_HEIGHT-100,z=0}
			local alpha=1
			...
			menubutton.displayobject.drawable:SetScript("OnClick",function(self,button,down)			
					if(button=="LeftButton" and down)then
						if(not XPRACTICE.MULTIPLAYERV2.IsRealOfficer("player"))then
							scenario.statuslabel:SetText("Must be group officer, or playing solo, to do that.\n(If you're in a raid group, you can switch to solo mode using the menu on the left.)",3.0)
							return
						end				
						if(scenario.gameinprogress)then
							scenario.statuslabel:SetText("Seems to be a game in progress already.",3.0)
							return
						end
						local args={string.char(i),
						XPRACTICE.SMOLDERON.RING_RADIUS[1][1],XPRACTICE.SMOLDERON.RING_RADIUS[1][2],
						XPRACTICE.SMOLDERON.RING_RADIUS[2][1],XPRACTICE.SMOLDERON.RING_RADIUS[2][2],
						XPRACTICE.SMOLDERON.RING_RADIUS[3][1],XPRACTICE.SMOLDERON.RING_RADIUS[3][2],
						XPRACTICE.SMOLDERON.RING_RADIUS[4][1],XPRACTICE.SMOLDERON.RING_RADIUS[4][2],
						XPRACTICE.SMOLDERON.RING_RADIUS[5][1],XPRACTICE.SMOLDERON.RING_RADIUS[5][2],
						XPRACTICE.SMOLDERON.PLAYER_HITBOX_RADIUS}
						scenario.multiplayer:FormatAndSend("LOCK")
						scenario.multiplayer:FormatAndSendCustom("WORLD_IN_FLAMES",unpack(args))
					end
				end)
			menubutton:SetText("Pattern "..tostring(i))
			menubutton:SetSize(150,60)
			tinsert(self.startbuttons,menubutton)
		end		
		
		for i=1,#self.stopbuttons do
			self.stopbuttons[i]:Die()	
		end; self.stopbuttons={}	
		
	end
	
	function class:SwitchToStopButton()
		local scenario=self
		self.statuslabel:SetText("",10.0)
		self.stopbuttons={}
		for i=1,1 do
			...
			menubutton=XPRACTICE.Button.new()
			menubutton:Setup(self.game.environment_gameplay)		
			menubutton.position={x=self.game.SCREEN_WIDTH/2-200+200*(i-0)-150/2,y=self.game.SCREEN_HEIGHT-100,z=0}
			local alpha=1
			menubutton:SetLockedStartingAlpha(alpha)
			menubutton:EnableMouse(true)
			menubutton.scenario=self
			menubutton.displayobject.drawable:SetScript("OnClick",function(self,button,down)			
					if(button=="LeftButton" and down)then
						if(not XPRACTICE.MULTIPLAYERV2.IsRealOfficer("player"))then
							scenario.statuslabel:SetText("Must be group officer, or playing solo, to do that.",3.0)
							return
						end				
						if(not scenario.gameinprogress)then
							scenario.statuslabel:SetText("There doesn't seem to be a game in progress, but we'll send the cancel request anyway...",3.0)
						end
			...
			menubutton:SetSize(150,60)
			tinsert(self.stopbuttons,menubutton)
		end			
	
	
		for i=1,#self.startbuttons do
			self.startbuttons[i]:Die()	
		end; self.startbuttons={}	
	...
	function class:Step(elapsed)
		...
		-- if(self.gameinprogress)then
			-- self.replaytime=self.replaytime+elapsed
		-- end
	...
	function class:Brez()
		local auras=self.player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.SMOLDERON.Aura_ChangePhaseForced)
		if(#auras>0)then 			
			return 
		end				
	...
	function class:BrezOther(player)
		local auras=player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.SMOLDERON.Aura_ChangePhaseForced)
		if(#auras>0)then 			
			return 
		end				
	...
	function class:PlaceHintArrow(x,y)
		...
		local arrow=XPRACTICE.SMOLDERON.HintArrow.new()
	...
	function class:PlaceBossHintArrow(x,y)
		...
		local arrow=XPRACTICE.SMOLDERON.BossHintArrow.new()
	...
	function class.QuickCollapseButton(environment,scenario,buttongroup,text,statuslabeltext,statuslabelduration)
		statuslabelduration=statuslabelduration or 3.0
		local menubutton
		menubutton=XPRACTICE.Button.new()
					...
					for i=1,#buttongroup do
						...
						if(mb==menubutton and not mb.selected)then
							if(mb.buttons)then
								...
								mb:Select(true)
								if(statuslabeltext)then
									scenario.statuslabel:SetText(statuslabeltext,statuslabelduration)
								end
							end
						else
							if(mb.buttons)then
	...
	XPRACTICE.ScenarioList.RegisterScenario(XPRACTICE.SMOLDERON.Scenario,"World in Flames","(Mythic Smolderon)","SCENARIO")
	XPRACTICE.ScenarioList.RegisterScenario(XPRACTICE.AMIRDRASSILRAIDMAPS.Scenario,"10.2","Amirdrassil, the Dream's Hope","SEPARATOR")
end
