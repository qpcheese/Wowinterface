do
	...
	do
		...
		XPRACTICE.VOTIRAIDMAPS.Floor=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.VOTIRAIDMAPS.Floor
		...
		function class:TransitionTo(index)
			if(XPRACTICE.VOTIRAIDMAPS.FloorData.DrawMap[index])then
				...
				XPRACTICE.VOTIRAIDMAPS.Bosses.CreateBosses(self.scenario,index)
			...
			if(self.transitionout)then
				...
				if(not stillinprogress)then	
					...
					if(self.transitionpending)then
						...
						if(XPRACTICE.VOTIRAIDMAPS.FloorData.DrawMap[self.transitioninindex])then
							for i=1,#self.actors do
						...
						XPRACTICE.VOTIRAIDMAPS.FloorData.DrawMap[self.transitioninindex](self.environment,self)
