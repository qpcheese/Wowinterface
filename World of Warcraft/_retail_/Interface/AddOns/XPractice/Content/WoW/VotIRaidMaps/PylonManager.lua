...
--TODO: phase 2+3 area denial hex effects around pillars
	...
	XPRACTICE.VOTIRAIDMAPS.Pylon = {}
	XPRACTICE.VOTIRAIDMAPS.Pylon.__index = XPRACTICE.VOTIRAIDMAPS.Pylon
	function XPRACTICE.VOTIRAIDMAPS.Pylon.new()
		local self=setmetatable({}, XPRACTICE.VOTIRAIDMAPS.Pylon)	
		return self
	end
	local class=XPRACTICE.VOTIRAIDMAPS.Pylon
...
do
	...
	XPRACTICE.VOTIRAIDMAPS.PylonManager=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.VOTIRAIDMAPS.PylonManager
	...
	function class:NewPylon(x,y)
		local pylon=XPRACTICE.VOTIRAIDMAPS.Pylon.new()
					...
					elseif(pylon.pylonstate>pylon.targetpylonstate)then
						...
						if(prevstate>0.4 and pylon.pylonstate<=0.4)then
							if(pylon.tyranny)then
								pylon.tyranny=false
								local orb=XPRACTICE.VOTIRAIDMAPS.TyrannyOrb.new()
