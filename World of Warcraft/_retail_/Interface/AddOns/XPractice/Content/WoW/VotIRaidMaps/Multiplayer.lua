do	
	...
	local roomnumber=XPRACTICE.VOTIRAIDMAPS.NPCID
	local roomBCD=XPRACTICE.MULTIPLAYERV2.Datatypes.BCD.ToString(XPRACTICE.VOTIRAIDMAPS.NPCID)	
	...
	XPRACTICE.VOTIRAIDMAPS.Multiplayer=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.VOTIRAIDMAPS.Multiplayer		
	...
	function class.Handle_HELLO(self,sender,args)
		...
		--print(XPRACTICE.MULTIPLAYERV2.Multiplayerv2.Handle, XPRACTICE.VOTIRAIDMAPS.Handle)
	...
	function class.HandleCustom_SPRINT(self,sender,args)
		...
		local visual=XPRACTICE.VOTIRAIDMAPS.SprintVisualEffect.new()
	...
	function class.HandleCustom_ROAR(self,sender,args)
		...
		if(distsqr<=radius*radius)then
			...
			local visual=XPRACTICE.VOTIRAIDMAPS.RoarVisualEffect.new()
	...
	function class.HandleCustom_PALLYBUBBLE(self,sender,args)
		...
		if(#auras==0)then
			local aura=player.combatmodule:ApplyAuraByClass(XPRACTICE.VOTIRAIDMAPS.Aura_PallyBubble)
		end
		local auras=player.combatmodule.auras:GetAurasByClassIfExist(XPRACTICE.VOTIRAIDMAPS.Aura_Freeze)
		for i=1,#auras do
			auras[i]:Die()
		end
	end	
	...
	function class.HandleCustom_CHANGEMAP(self,sender,args)
		...
		local name=tostring(XPRACTICE.VOTIRAIDMAPS.ChangeMap.BossNames[i])
		if(name)then
