...
do
	...
	XPRACTICE.VOTIRAIDMAPS.AnimationList={}
	----XPRACTICE.AnimationTemplate.QuickSetup(animationlist,			name,		index,	subindex,	priority,	duration,	nextanimation)
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"IdleAttack1H",17,		0,	"Shuffle",	1.5,		nil)
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"IdleAttack1H_2",17,		1,	"Shuffle",	1.5,		nil)
	
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"Custom11",1500,		0,	"Active",	3.5,		nil)
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"Custom3",156,		0,	"Active",	1.0,		nil)
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"Custom2",155,		0,	"Active",	0.5,		XPRACTICE.VOTIRAIDMAPS.AnimationList.Custom3)
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"Custom1",154,		0,	"Active",	1.0,		XPRACTICE.VOTIRAIDMAPS.AnimationList.Custom2)
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"Custom0",153,		0,	"Active",	0.5,		XPRACTICE.VOTIRAIDMAPS.AnimationList.Custom1)
	
	XPRACTICE.AnimationTemplate.QuickSetup(XPRACTICE.VOTIRAIDMAPS.AnimationList,"ExplosiveRuneCustom",213,	0,		"Active",		nil,nil)
	

	do		
		--local super=XPRACTICE.Mob
		local super=XPRACTICE.MULTIPLAYERV2.Multiplayerv2Murloc
		XPRACTICE.VOTIRAIDMAPS.TheJailer=XPRACTICE.inheritsFrom(super)
		local class=XPRACTICE.VOTIRAIDMAPS.TheJailer
		...
		function class:Setup(environment,x,y,z)
			...
			-- self.animationmodule.PlayAnimation=function(self,animationtemplate)
				...
				-- if(jailer.attachedweapon)then					
					...
					-- --jailer.attachedweapon.animationmodule:PlayAnimation(XPRACTICE.VOTIRAIDMAPS.AnimationList.Custom11)
				-- end
			-- end
			--
			--self.animationmodule:PlayAnimation(XPRACTICE.VOTIRAIDMAPS.AnimationList.Custom11)
		...
		function class:CreateCombatModule()
			...
			local spell=XPRACTICE.VOTIRAIDMAPS.Spell_WorldCrusher.new();spell:Setup(self.combatmodule)
			self.combatmodule.spellbook.worldcrusher=spell
			local spell=XPRACTICE.VOTIRAIDMAPS.Spell_Torment1.new();spell:Setup(self.combatmodule)
			self.combatmodule.spellbook.torment1=spell					
			local spell=XPRACTICE.VOTIRAIDMAPS.Spell_ChainsOfOppression.new();spell:Setup(self.combatmodule)
			self.combatmodule.spellbook.chainsofoppression=spell
			local spell=XPRACTICE.VOTIRAIDMAPS.Spell_Martyrdom.new();spell:Setup(self.combatmodule)
			self.combatmodule.spellbook.martyrdom=spell			
			local spell=XPRACTICE.VOTIRAIDMAPS.Spell_ExplosiveRunes.new();spell:Setup(self.combatmodule)
			self.combatmodule.spellbook.explosiverunes=spell
			local spell=XPRACTICE.VOTIRAIDMAPS.Spell_RelentlessDomination.new();spell:Setup(self.combatmodule)
				...
				if(self.combatmodule.castedspell==nil and self.combatmodule.channeledspell==nil and not self.scenario.stopallevents)then
					...
					if(self.autoattacktimer<=0)then
						...
						if(math.random()<0.5)then
							self.animationmodule:PlayAnimation(XPRACTICE.VOTIRAIDMAPS.AnimationList.IdleAttack1H)
							if(self.attachedweapon)then
								self.attachedweapon.animationmodule:PlayAnimation(XPRACTICE.VOTIRAIDMAPS.AnimationList.IdleAttack1H)
							end
						else
							self.animationmodule:PlayAnimation(XPRACTICE.VOTIRAIDMAPS.AnimationList.IdleAttack1H_2)
							if(self.attachedweapon)then
								self.attachedweapon.animationmodule:PlayAnimation(XPRACTICE.VOTIRAIDMAPS.AnimationList.IdleAttack1H_2)
							end						
						end
					end
				end
