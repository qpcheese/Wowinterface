do
	...
	XPRACTICE.Aura_FlyToPosition = XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.Aura_FlyToPosition
	
	function class.QuickFlyToPosition(mob,destx,desty,speed,auraclass)
		--TODO: make this function inheritable somehow
		speed=speed or 28
		auraclass=auraclass or XPRACTICE.Aura_FlyToPosition
		local aura=mob.combatmodule:ApplyAuraByClass(auraclass)
		aura.scenario=mob.scenario
		aura.destination={x=destx,y=desty,z=0}
		local dist=math.sqrt(XPRACTICE.distsqr(mob.position.x,mob.position.y,destx,desty))
		local duration=dist/speed		
		aura.expirytime=duration
		local yaw=math.atan2(desty-mob.position.y,destx-mob.position.x)
	...
	function class:GetGripPosition()
		local denominator=(self.expirytime)
		if(denominator<=0)then denominator=0.1 end
		local ratio=(self.localtime)/denominator
