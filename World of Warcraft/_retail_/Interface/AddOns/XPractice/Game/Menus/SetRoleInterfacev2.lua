do
	...
	XPRACTICE.SetRoleInterfacev2=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.SetRoleInterfacev2
	...
	function class:CreateAssociatedObjects()
		...
		for i=1,#scenario.roles do
			...
			button.displayobject.drawable:SetScript("OnClick",function(self,button,down)
					if(button=="LeftButton" and down)then						
						...
						elseif(false)then
							...
							XPRACTICE_SAVEDATA.THEJAILER2.roleid=role.roleid
			...
			if(role.requireofficer)then
				desc="(Experimental.  Requires lead/assist)"
		...
		button.position.x=OFFSETX+game.window.frame:GetWidth()/2-150
		button.position.y=OFFSETY+game.window.frame:GetHeight()/2
		button:SetText("Spectator")		
		button.displayobject.drawable:SetScript("OnClick",function(self,button,down)
				if(button=="LeftButton" and down)then
					local rolechar="X"
					XPRACTICE_SAVEDATA.THEJAILER2.roleid="X"
					if(not scenario.player.rolepointer)then
						scenario.statuslabel:SetText("You are already spectating.")
					else
						scenario.multiplayer:FormatAndSend("CHANGEROLE","X")
					end
					scenario.tempmenu:UnloadCurrentMenu()					
				end
			end)			
		...
		text.position.y=OFFSETY+game.window.frame:GetHeight()/2
		text:SetSize(300,30)
		text:SetText("Return to spectator mode.")
		--text:SetText("")
		tinsert(self.labels,text)				
		...
		text.position.y=OFFSETY+game.window.frame:GetHeight()/2
		text:SetSize(300,30)
		text:SetText("Cancel role selection.")
		--text:SetText("")
		tinsert(self.labels,text)	
